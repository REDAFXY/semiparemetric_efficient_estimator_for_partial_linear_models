...
for i in range(errornum):
    for j in range(ntimes):
        random.seed(j) 
        np.random.seed(j)
        print('errror'+str(i)+'-result'+str(j)+'-th.pickle')
        resulterri = predsim(i,par_1_default,par_2_default)
        #save
        list_file = open('errror'+str(i)+'-result'+str(j)+'-th.pickle','wb')
...
for i in range(errornum):
    ...
    for j in range(ntimes):
        list_file = open('errror'+str(i)+'-result'+str(j)+'-th.pickle','rb')
    ...
    list_file = open('errror'+str(i)+'result'+'-allpickle','wb')
...
import pickle
for i in range(errornum):
    list_file = open('errror'+str(i)+'result'+'-allpickle','rb')
    resulterri = pickle.load(list_file)
...
list_file = open('simhigh.pickle','wb')
...
#read
list_file = open('simhigh.pickle','rb')
